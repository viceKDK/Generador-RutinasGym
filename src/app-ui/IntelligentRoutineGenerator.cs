...
namespace GymRoutineGenerator.UI
{
    ...
    public List<WorkoutDay> LastGeneratedWorkoutPlan { get; private set; } = new List<WorkoutDay>();


        public IntelligentRoutineGenerator()
        {
            random = new Random();
            imageDatabase = new SQLiteExerciseImageDatabase();
        ...
        public string GeneratePersonalizedRoutine(UserProfile profile)
        {
            ...
            LastGeneratedWorkoutPlan = workoutPlan;
        ...
        private List<WorkoutDay> CreatePersonalizedWorkout(UserProfile profile)
        {
            ...
            var fitnessLevel = (profile.FitnessLevel ?? string.Empty).Trim();
            var isBeginner = string.Equals(fitnessLevel, "Principiante", StringComparison.OrdinalIgnoreCase);
            var isIntermediate = string.Equals(fitnessLevel, "Intermedio", StringComparison.OrdinalIgnoreCase);

            if (profile.Age >= 50 && isBeginner)
            {
                // Rutina para adultos mayores principiantes
                workoutDays = CreateSeniorBeginnerWorkout(profile);
            }
            else if (profile.Age >= 40 && isBeginner)
            {
        ...
        private string AnalyzeProfile(UserProfile profile)
        {
            ...
            bool isBeginner = profile.FitnessLevel.Contains("Principiante") || profile.FitnessLevel.Contains("Beginner");
    ...
    public class WorkoutDay
    {
        ...
        public string[] MuscleGroups { get; set; } = new string[0]; // Grupos musculares del día
    }
